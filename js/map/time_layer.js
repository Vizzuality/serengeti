...
L.TimeLayer = L.CanvasLayer.extend({
  ...
  options: {
    ...
    geometry_table: 'sitecoords2013',
    steps: window.AppData.STEPS,
    step: window.AppData.STEP,
    resolution:1,
  ...
  get_time_data: function (coord, zoom) {
    ...
    " ) " +
    " SELECT  " +
    "    x, y, array_agg(c) spp, array_agg(d) dates " +
    " FROM ( " +
    "    SELECT " +
    "      round(CAST (st_xmax(hgrid.cell) AS numeric),4) x, round(CAST (st_ymax(hgrid.cell) AS numeric),4) y, " +
    "      sum(i.spp) c, floor((date_part('epoch',{1})- {2})/{3}) d ".format(self.options.countby, self.options.column, self.options.start_date, self.options.step) +
    "    FROM " +
    "        hgrid, {0} i, {1} j".format(self.options.table, self.options.geometry_table) +
    "    WHERE " +
    "        " + ( window.AppData.SPECIE ? (" species ILIKE '%" + window.AppData.SPECIE + "%' AND ") : "" ) + 
    ...
    "        AND ST_Intersects(j.the_geom_webmercator, hgrid.cell) " +
    "        AND i.{0} = j.{0}".format("site_code") +
    "    GROUP BY " +
  ...
  pre_cache_months: function (rows, coord, zoom) {
    ...
    for (var i in rows) {
        ...
        for (var j = 0; j < row.dates.length; ++j) {
            values[base_idx + row.dates[j]] = Math.min(row.spp[j]*window.BALL_SIZE_GAIN, 30);
        }
    }
  ...
  setTime: function(d) {
    this.realTime = new Date(d).getTime()/(window.AppData.STEP*1000) - this.options.start_date/(window.AppData.STEP) - 4; // one hour before
    this.time = this.realTime>>0;
  },

  set_time: function(t) {
    this.realTime = new Date(t).getTime()/(window.AppData.STEP) - this.options.start_date/(window.AppData.STEP) - 4; // one hour before
    this.time = this.realTime >> 0;
  },
  ...
  getTime: function() {
    return new Date(1000*(this.options.start_date + this.realTime*window.AppData.STEP));
  }
});
